        ...
        if decoded_msg == "ENDUPLOAD":
            ...
            try:
                ...
                if upload_filename == "main.py":
                    sp.send("🔄 Restarting to load new main.py...")
                    import machine
                    machine.reset()
            except Exception as e:
            ...
            upload_lines.append(decoded_msg)
            sp.send("Line received for upload")
        return
    ...
    if relay_is_on:
        ...
        for event in SCHEDULED_EVENTS:
            ...
            if (current_time[0] == y and current_time[1] == m and current_time[2] == d and
                ...
                print("Relay ON at " + timestamp + " for {} min".format(duration))
